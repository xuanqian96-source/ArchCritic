...
- `frontend-v1-replica/site.config.json`、`vite.config.ts`：管理公开统计标识及域名，并在正式构建时把官方加载代码写入首页头部，供安装检查器直接读取。
- `frontend-v1-replica/src/analytics.ts`：复用头部安装的统计实例，按固定路由手动记录 PV；由 `main.tsx` 启动，不二次安装脚本。
...
- 百度统计 ID 属于公开站点标识，现由前端站点配置在构建时注入 HTML，避免百度检查器无法识别运行时安装；原后端配置接口保留兼容。统计按单页路由手动发送，初始加载与页面切换去重。
